...
public class LayoutConstraints {
	...
	public static final LayoutConstraint prefSizeRange = new LayoutConstraint() {
		...
		public boolean isViolated(MuisElement container, Dimension size, LayoutScratchPad layout) {
			for(MuisElement child : layout) {
				Rectangle bounds = layout.get(child);
				if(bounds.width < LayoutUtils.getSize(child, horizontal, minPref, size.width, size.height, false, null))
					return true;
				if(bounds.width > LayoutUtils.getSize(child, horizontal, maxPref, size.width, size.height, false, null))
					return true;
				if(bounds.height < LayoutUtils.getSize(child, vertical, minPref, size.height, size.width, false, null))
					return true;
				if(bounds.height > LayoutUtils.getSize(child, vertical, maxPref, size.height, size.width, false, null))
					return true;
			}
			return false;
		}
		...
		public float getViolation(MuisElement container, Dimension size, LayoutScratchPad layout) {
			...
			for(MuisElement child : layout) {
				Rectangle bounds = layout.get(child);
				if(bounds.width < child.bounds().get(horizontal).getGuide().getMinPreferred(bounds.height, false))
					ret += child.bounds().get(horizontal).getGuide().getMinPreferred(bounds.height, false) - bounds.width;
				if(bounds.width > child.bounds().get(horizontal).getGuide().getMaxPreferred(bounds.height, false))
					ret += bounds.width - child.bounds().get(horizontal).getGuide().getMaxPreferred(bounds.height, false);
				if(bounds.height < child.bounds().get(vertical).getGuide().getMinPreferred(bounds.width, false))
					ret += child.bounds().get(vertical).getGuide().getMinPreferred(bounds.width, false) - bounds.height;
				if(bounds.height > child.bounds().get(vertical).getGuide().getMaxPreferred(bounds.width, false))
					ret += bounds.height - child.bounds().get(vertical).getGuide().getMaxPreferred(bounds.width, false);
			}
			return ret;
		}
	};
