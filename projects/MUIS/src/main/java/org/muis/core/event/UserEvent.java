...
public class UserEvent extends MuisEvent<Void> {
	...
	/**
	 * Cancels this event so that it will not be propagated to any more elements. This method should be called when a listener knows what
	 * the event was intended for by the user and has the capability to perform the intended action. In this case, the event should not be
	 * propagated further because its purpose is complete and any further action would be undesirable.
	 */
	public void cancel() {
		isCanceled = true;
	}
