...
public class WrappingDocumentModel {
	private volatile QuickDocumentModel theWrapped;
	private volatile QuickDocumentModel theWrapper;

	private final Collection<QuickDocumentModel.ContentListener> theContentListeners;
	private final Collection<QuickDocumentModel.StyleListener> theStyleListeners;
	...
	public WrappingDocumentModel(QuickDocumentModel model) {
	...
	public QuickDocumentModel getWrapped() {
		return theWrapped;
	}
	 ...
	 */
	public void setWrapped(QuickDocumentModel model) {
		QuickDocumentModel oldModel = model;
		ArrayList<QuickDocumentModel.ContentListener> cls = new ArrayList<>(theContentListeners);
		ArrayList<QuickDocumentModel.StyleListener> styleLs = new ArrayList<>(theStyleListeners);
		...
		theWrapped = model;
		for(QuickDocumentModel.ContentListener cl : cls) {
			oldModel.removeContentListener(cl);
			model.addContentListener(cl);
		}
		for(QuickDocumentModel.StyleListener styleL : styleLs) {
	...
	private class InternalSimpleDocumentModel implements QuickDocumentModel {
		private final QuickDocumentModel theInternalWrapped;

		InternalSimpleDocumentModel(QuickDocumentModel wrap) {
			theInternalWrapped = wrap;
		}

		QuickDocumentModel doc() {
