		...
		events().filterMap(MouseEvent.mouse).act(new org.muis.rx.Action<MouseEvent>() {
			...
			void drawPoint(MouseEvent mEvt) {
				if(isMouseDown) {
					...
					try {
						theImage.setRGB(pos.x, pos.y, getStyle().getSelf().get(org.muis.core.style.FontStyle.color).get().getRGB());
	...
	public void paintSelf(Graphics2D graphics, Rectangle area) {
		...
		if(theImage != null) {
			graphics.drawImage(theImage, 0, 0, theImage.getWidth(), theImage.getHeight(),
				getStyle().getSelf().get(org.muis.core.style.BackgroundStyle.color).get(), null);
		}
	}
