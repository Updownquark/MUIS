...
public class DefaultStyleParser implements QuickStyleParser {
	...
	@Override
	public ParsedStyleSheet parseStyleSheet(QuickParseEnv parseEnv, QuickToolkit toolkit, URL location)
		...
		stack.push();
		addNamespaces(rootEl, location, stack, parseEnv.msg());
		for(Element child : rootEl.getChildren()){
			parseStyleElement(child, location, parseEnv, stack, styleSheet);
		}
		return null;
	}
	...
	private void parseStyleElement(Element xml, URL location, QuickParseEnv parseEnv, ExpressionContextStack stack, ParsedStyleSheet styleSheet) {
		stack.push();
		...
		case "domain":
			for(Element child : xml.getChildren()){
				...
				applyStyleValue(name, attr, valueStr, parseEnv, styleSheet, stack);
			}
			break;
		case "attr":
			String domain = xml.getAttributeValue("domain");
			String valueStr = xml.getAttributeValue("value");
			applyStyleValue(domain, name, valueStr, parseEnv, styleSheet, stack);
		}
	}
	...
	private void applyStyleValue(String name, String attr, String valueStr, QuickParseEnv parseEnv, ParsedStyleSheet styleSheet,
