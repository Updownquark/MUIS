...
import org.wam.core.WamException;
...
public class Colors
{
	...
	/**
	 * "Lime" is basic green from HTML. "Green" is a darker color. #00ff00 <font
	 * color="green">######</font>.
	 */
	...
	/**
	 * A darker green than {@link java.awt.Color#green}. Use {@link #lime} for that green.
	 * #008000<font color="#008000">######</font>
	 */
	 ...
	 *            <ul>
	 *            <li>Hexadecimal RGB: Looks like #XXXXXX where each 'X' is a hexadecimal digit (0-9
	 *            or a-f)</li>
	 *            <li>Hexadecimal HSB: Looks like $XXXXXX where each 'X' is a hexadecimal digit (0-9
	 *            or a-f)</li>
	 ...
	 *            <li>A named color matching the name of one of this class's static color constant
	 *            fields</li>
	 *            </ul>
	 *            The value is not case-sensitive.
	 * @return The color corresponding to the string
	 * @throws WamException If the color is unrecognized or cannot be parsed
	 */
	public static Color parseColor(String str) throws WamException
	{
		str = str.toLowerCase();
		if(str.startsWith("#"))
		{
			...
			if(!str.matches("[0-9a-f]{6}"))
				throw new WamException(
					"RGB colors must be in the form of #XXXXXX where X is 0-9 or a-f");
		...
		else if(str.startsWith("$"))
		{
			...
			if(!str.matches("[0-9a-f]{6}"))
				throw new WamException(
					"HSB colors must be in the form of #XXXXXX where X is 0-9 or a-f");
			return Color.getHSBColor(hexInt(str, 0) / 255.0f, hexInt(str, 2) / 255.0f, hexInt(str,
				4) / 255.0f);
		}
		else if(str.startsWith("rgb("))
		{
			...
			if(str.charAt(str.length() - 1) != ')')
				throw new WamException("Colors that start with 'rgb(' must end with ')': " + orig);
			...
			try
			{
				int idx = str.indexOf(',');
				if(idx < 0)
					throw new WamException("Colors that start with 'rgb('"
						+ " must have 3 integers separated by commas: " + orig);
		...
		else if(str.startsWith("hsb("))
		{
			String orig = str;
			if(str.charAt(str.length() - 1) != ')')
				throw new WamException("Colors that start with 'hsb(' must end with ')': " + orig);
			str = str.substring(4, str.length() - 1);
			int h, s, b;
			try
			{
				int idx = str.indexOf(',');
				if(idx < 0)
					throw new WamException("Colors that start with 'hsb('"
						+ " must have 3 integers separated by commas: " + orig);
		...
		else
		{
			...
			if(ret == null)
				throw new WamException("No color named " + str);
			return ret;
		}
	}
