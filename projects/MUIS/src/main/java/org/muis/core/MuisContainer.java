 ...
 * elements. The content of a container may be different than its {@link MuisElement#getChildren() children}. An example of a container is a
 ...
 * display. A combo box's {@link MuisElement#getChildren() children} will have the content pane, the button, and the menu; but its
 ...
 * MuisContainer implementation; and it might also wish to expose the widgets that represent its tabs. It might provide a getTabs() method
 * that returns a MuisContainer implementation whose type is more specific than just MuisElement. In this case, the getTabs() method might
 * return the tab container widget, or it might simply return MuisContainer&lt;TabWidget&gt;, whose implementation is undefined and does not
 * extend MuisElement, thus exposing the set of tab widgets without exposing any more of the tab container's inner structure.
 * </p>
 *
 * @param <E> The type of element contained in this container
 */
public interface MuisContainer<E extends MuisElement> {
	/**
	 * @return The logical contents of this container
	 * @see MuisContainer
	 */
	org.muis.core.mgr.ElementList<E> getContent();
}
