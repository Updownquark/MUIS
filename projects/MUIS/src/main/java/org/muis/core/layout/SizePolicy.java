...
public interface SizePolicy {
	 ...
	 * @param crossSize The hypothetical size of the widget in the opposite dimension
	 * @return The size guide setting of the given type in this policy
	 */
	int get(LayoutGuideType type, int crossSize);

	/**
	 * @return The stretch factor for the widget, between 0 and 1. This determines how layouts will distribute extra space if there is
	 *         leftover space after all content has been given its preferred size. Larger values mean the widget can make better use of
	 *         extra space than other widgets. A value of 0 means that the widget may be given extra space (up to its max size) but with no
	 *         advantage at all. A value of 1 means the widget is eager to stretch out to its maximum.
	 */
	float getStretch();
}
