...
public class KeyBoardEvent extends UserEvent {
	 ...
	 */
	public KeyBoardEvent(QuickWidgetDocument doc, QuickWidget target, KeyCode keyCode, Set<MouseEvent.ButtonType> pressedButtons,
		Set<KeyBoardEvent.KeyCode> pressedKeys, boolean pressed, Object cause) {
	...
	private KeyBoardEvent(KeyBoardEvent backing, QuickWidget widget) {
	...
	@Override
	public KeyBoardEvent copyFor(QuickWidget widget) {
		if (!QuickWidgetUtils.isAncestor(widget, getTarget()))
			throw new IllegalArgumentException("This event (" + this + ") is not relevant to the given element (" + widget + ")");
		return new KeyBoardEvent(this, widget);
	}
	...
	public String toString() {
		return "Keyboard event: " + theKeyCode + (wasPressed ? " pressed " : " released ") + " for " + getWidget();
	}
}
