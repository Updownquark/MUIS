...
public class LayoutUtils {
	...
	/**
	 * @param widget The widget to get the intended size of
	...
	public static int getSize(QuickWidget<?> widget, Orientation orientation, LayoutGuideType type, int parallelSize, int crossSize,
		...
		Size minSize = widget.getElement().atts().getValue(LayoutAttributes.getSizeAtt(orientation, LayoutGuideType.min), null);
		Size maxSize = widget.getElement().atts().getValue(LayoutAttributes.getSizeAtt(orientation, LayoutGuideType.max), null);
		ret = widget.getElement().atts().getValue(att, null);
		...
		if(ret != null) {
			...
			int size = widget.bounds().get(orientation).getGuide()//
	...
	/**
	 * @param widget The widget to get the size for
	...
	public static Size[] getLayoutSize(QuickWidget<?> widget, Orientation orientation, LayoutGuideType type, int crossSize, boolean csMax) {
		...
		if (ret != null)
			return new Size[] { ret };
		Size minSize = widget.getElement().atts().get(LayoutAttributes.getSizeAtt(orientation, LayoutGuideType.min)).get();
		Size maxSize = widget.getElement().atts().get(LayoutAttributes.getSizeAtt(orientation, LayoutGuideType.max)).get();
		if (minSize != null && maxSize != null && minSize.compareTo(maxSize) >= 0)
			return new Size[] { minSize };
		Size layoutSize = new Size(widget.bounds().get(orientation).getGuide()//
