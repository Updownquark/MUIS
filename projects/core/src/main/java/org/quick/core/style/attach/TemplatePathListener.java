...
public class TemplatePathListener {
	...
	private java.util.Map<QuickAttribute<AttachPoint>, TemplateListenerValue> theAttachParents;
	 ...
	 */
	public void listen(QuickElement element) {
		if(theElement != null)
	...
	private void roleChanged(QuickAttribute<QuickTemplate.AttachPoint> roleAttr, AttachPoint role) {
		QuickTemplate newAttachParent = getAttachParent(role);
	...
	private QuickTemplate getAttachParent(AttachPoint role) {
		Class<? extends QuickElement> definer = role.template.getDefiner();
		QuickElement parent = theElement.getParent();
		while(parent != null && !definer.isInstance(parent))
			parent = parent.getParent();
		return (QuickTemplate) parent;
	}
