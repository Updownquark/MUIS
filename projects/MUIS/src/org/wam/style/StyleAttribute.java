...
/**
 * A style property that can affect the rendering of WAM elements
 * 
...
public final class StyleAttribute<T>
{
	 ...
	 * @throws WamException If the value cannot be parsed
	 */
	public T parse(String value) throws WamException
	{
		switch(type)
		{
		case BOOLEAN:
			if("true".equals(value))
				...
				throw new WamException("Value \"" + value + "\" for boolean property "
		...
		case INT:
			...
			{
				throw new WamException("Value \"" + value + "\" for integer property "
		...
		case FLOAT:
			...
			{
				throw new WamException("Value \"" + value + "\" for real number property "
		...
		case ENUM:
			for(T val : javaType.getEnumConstants())
				if(((Enum<?>) val).name().equals(value))
					return val;
			throw new WamException("Value \"" + value + "\" for enumerated property "
				+ domain.getName() + "." + name + " does not match any valid values");
		case COLOR:
			return (T) Colors.parseColor(value);
		}
		throw new WamException("Unrecognized attribute type: " + type);
	}
