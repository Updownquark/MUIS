...
/** The default implementation of {@link QuickValueReferenceParser} */
public class DefaultModelValueReferenceParser implements QuickValueReferenceParser {
	...
	private QuickValueReferenceParser theSuperParser;
	 ...
	 */
	public DefaultModelValueReferenceParser(QuickValueReferenceParser superParser, QuickClassView classView) {
	...
	public QuickValueReferenceParser getSuperParser() {
	...
	@Override
	public ObservableValue<?> parse(String mvr, boolean asType) throws QuickParseException {
		ParsedItem item;
		try {
			ParseMatch [] matches = theParser.parseMatches(mvr);
			if(matches.length != 1)
				throw new QuickParseException("Exactly one expression expected for property value: " + mvr);
			item = theParser.parseStructures(new prisms.lang.ParseStructRoot(mvr), matches)[0];
		} catch(ParseException e) {
			throw new QuickParseException("Property parsing failed: " + mvr, e);
		}
		try {
