...
import org.wam.core.event.*;
import org.wam.style.NamedStyleGroup;

import prisms.util.ArrayUtils;

/**
 * Contains all data pertaining to a WAM application
 */
public class WamDocument implements WamMessage.WamMessageCenter
{
	/**
	...
	/**
	 * Allows a WAM document to retrieve graphics to draw itself on demand
	 */
	...
	private org.wam.parser.WamParser theParser;

	private WamToolkit theDefaultToolkit;

	private WamClassView theClassView;

	private WamHeadSection theHead;
	...
	private WamElement theMousedOver;
	 ...
	 */
	public WamDocument(GraphicsGetter graphics)
	{
		theHead = new WamHeadSection();
	 ...
	 * @param defaultToolkit The default toolkit to load core WAM classes with
	 */
	public void initDocument(org.wam.parser.WamParser parser, WamToolkit defaultToolkit)
	{
		if(theParser != null)
			throw new IllegalArgumentException("Cannot initialize a document twice");
		theParser = parser;
		theClassView = new WamClassView(this);
		theHead = new WamHeadSection();
	 ...
	 */
	public org.wam.parser.WamParser getParser()
	{
		return theParser;
	}

	/**
	 * @return The default toolkit to load core WAM classes from
	 */
	public WamToolkit getDefaultToolkit()
	{
	 ...
	 */
	public WamClassView getClassView()
	{
		return theClassView;
	}
	...
	public void removeGroup(String name)
	{
		...
		for(NamedStyleGroup group : theDocumentGroups)
			if(group.getName().equals(name))
			{
				for(WamElement el : group.members())
	 ...
	 */
	public void message(WamMessage.Type type, String text, Throwable exception, Object... params)
	{
		WamMessage message = new WamMessage(this, type, theRoot.getStage(), text, exception, params);
	 ...
	 */
	public void removeMessage(WamMessage message)
	{
	...
	/**
	 * @return The worst type of message associated with the WAM document
	 */
	public WamMessage.Type getWorstMessageType()
	{
	...
	public final WamMessage [] getAllMessages()
	{
		...
		if(theRoot != null)
			for(WamMessage msg : theRoot.getAllMessages())
				ret.add(msg);
	...
	/**
	 * Renders this WAM document in a graphics context
	 * 
		...
		MouseEvent.ButtonType buttonType, int clickCount)
	{
		...
		theMouseX = x;
		theMouseY = y;
		WamElement element = theRoot.deepestChildAt(x, y);
		MouseEvent evt;
		switch(type)
		{
		case MOUSE_MOVED:
	...
	private void focusByMouse(WamElement element, int x, int y)
	{
		...
		WamElement lastChild = element;
		element = element.getParent();
		while(element != null)
		{
			Point relPos = element.getDocumentPosition();
			WamElement [] children = element.childrenAt(x - relPos.x, y - relPos.y);
			children = WamElement.sortByZ(children);
	...
	public void checkMouse()
	{
		...
		{
			WamElement [] branch = WamUtils.getBranchPoint(theMousedOver, element);
			if(branch != null)
			{
				if(branch[1] != null)
	 ...
	 * {@link WamElement#isFocusable()}, so this will work on any element.
	 * 
	 * @param toFocus The element to give the focus to
	 */
	public void setFocus(WamElement toFocus)
	{
		if(toFocus != theFocus)
		{
			if(theFocus != null)
				theFocus.fireUserEvent(new org.wam.core.event.FocusEvent(this, theFocus, false));
			theFocus = toFocus;
			if(theFocus != null)
				theFocus.fireUserEvent(new org.wam.core.event.FocusEvent(this, theFocus, true));
		}
	}
	...
	public void backupFocus()
	{
		...
		WamElement deepest = getDeepestElement(theRoot, false);
	...
	public void advanceFocus()
	{
		...
		/* If we get here, then there was no previous focusable element.  We must wrap around to the
		 * last focusable element. */
		WamElement deepest = getDeepestElement(theRoot, true);
		searchFocus(deepest, true);
		fireEvents();
	}

	boolean searchFocus(WamElement el, boolean forward)
	{
		WamElement lastChild = theFocus;
		WamElement parent = theFocus.getParent();
		while(parent != null)
		{
			WamElement [] children = parent.getChildren();
			WamElement [] forLoop = WamElement.sortByZ(children);
			if(!forward)
			{
				if(forLoop == children)
			...
			for(int c = 0; c < children.length; c++)
			{
				if(foundLastChild)
				{
					WamElement deepest = getDeepestElement(children[c], forward);
	...
	static WamElement getDeepestElement(WamElement root, boolean first)
	{
	...
	{
		org.wam.core.event.KeyBoardEvent evt = null;
		if(theFocus != null)
	...
	{
		org.wam.core.event.CharInputEvent evt = null;
		if(theFocus != null)
			evt = new org.wam.core.event.CharInputEvent(this, theFocus, c);
		else
			evt = new org.wam.core.event.CharInputEvent(this, theRoot, c);
		if(theFocus != null)
			theFocus.fireUserEvent(evt);
		fireEvents();
	}
