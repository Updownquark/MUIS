 ...
 * the document
 */
public class DocumentStyleSheet extends SimpleStyleSheet {
	/**
	...
	public DocumentStyleSheet() {
		...
		theDependencyListener = new StyleGroupTypeExpressionListener() {
			...
			public void eventOccurred(StyleGroupTypeExpressionEvent<?, ?> event) {
				if(isSet(DocumentStyleSheet.this, event.getAttribute(), event.getGroupName(), event.getType(), event.getExpression()))
					return;
