...
public class XmlStylesheetParser {
	...
	public XmlStylesheetParser() {
		...
		String command = null;
		try {
			command = "java.awt.Color rgb(double r, double g, double b){" + "return " + org.muis.core.style.Colors.class.getName()
				+ ".rgb(r, g, b);}";
	...
	public ParsedStyleSheet parse(Element element, MuisClassView classView, MuisMessageCenter msg) throws MuisParseException {
		...
		animLoop: for(Element animEl : element.getChildren("animate")) {
			...
			for(int i = 0; i < segments.length; i++) {
				...
				segments[i] = new AnimatedStyleSheet.AnimationSegment(endValue, 0, duration);
			}
