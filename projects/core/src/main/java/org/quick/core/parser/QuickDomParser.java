...
public class QuickDomParser implements QuickParser {
	...
	@Override
	public void fillToolkit(QuickToolkit toolkit) throws QuickParseException, IOException {
		Element rootEl;
		try {
				...
				for(Element dEl : el.getChildren()) {
					if(dEl.getName().equals("depends")) {
						...
						try {
							...
							throw new QuickParseException("Could not resolve or parse dependency " + dEl.getTextTrim() + " of toolkit "
								+ toolkit.getURI());
						}
						...
						try {
							...
							throw new QuickParseException("Could not resolve classpath " + dEl.getTextTrim() + " for toolkit \"" + name
								+ "\" at " + toolkit.getURI(), e);
						}
						try {
	...
	@Override
	public QuickDocumentStructure parseDocument(QuickEnvironment env, URL location, Reader reader) throws QuickParseException, IOException {
