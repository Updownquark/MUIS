...
public class PositionPropertyType extends QuickProperty.AbstractPropertyType<Position> implements
	QuickProperty.PrintablePropertyType<Position>
{
	...
	@Override
	public ObservableValue<? extends Position> parse(QuickParseEnv env, String value) throws QuickException {
		ObservableValue<?> modelValue = QuickProperty.parseExplicitObservable(env, value, false);
		if(modelValue != null) {
			if(modelValue.getType().canAssignTo(Position.class))
				...
				throw new QuickException("Model value " + value + ", type " + modelValue.getType() + " is not compatible with position.");
		}
		String number = value;
