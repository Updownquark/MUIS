...
public class MuisTextElement extends MuisLeaf
{
	...
	public SizePolicy getWSizer(int height)
	{
		...
		int lineIdx = 0;
		if(getStyle().get(org.muis.core.style.FontStyle.wordWrap))
		{
			int min = 0;
			int max = 0;
			int wordIdx = 0;
	...
	private int render(int width, Graphics2D graphics, int [] min)
	{
		...
		if(graphics != null)
		{
			...
			if(getStyle().get(org.muis.core.style.FontStyle.antiAlias))
				...
				for(int c = 0; c < theText.length(); c++)
				{
					...
					if(ch == ' ' || ch == '\n')
					{
						...
						while(lineW > width && c > breakIdx)
						{
							...
							{ // The first word is too long--break it where we can
